...
def build_generation_plan(request: dict[str, Any]) -> dict[str, Any] | None:
    ...
    print(f"[debug] build_generation_plan: request has 'directions'={'directions' in request} "
          f"value={directions!r}", file=sys.stderr)
    if not directions:
        print("[debug] build_generation_plan: returning None (no directions block)", file=sys.stderr)
        return None
...
def _run(args: argparse.Namespace):
    ...
    print(f"[debug] raw_request keys={sorted(raw_request.keys())}", file=sys.stderr)
    print(f"[debug] raw_request has 'directions'={'directions' in raw_request} "
          f"value={raw_request.get('directions')!r}", file=sys.stderr)
    print(f"[debug] args.directions={args.directions!r} args.mirror={args.mirror!r}", file=sys.stderr)
    ...
    print(f"[debug] raw_directions after CLI merge={raw_directions!r}", file=sys.stderr)
    directions = normalize_directions(raw_directions or None, states)
    print(f"[debug] normalize_directions returned={directions!r}", file=sys.stderr)
    if directions:
    ...
    print(f"[debug] request keys before build_generation_plan={sorted(request.keys())}", file=sys.stderr)
    plan = build_generation_plan(request)
    print(f"[debug] build_generation_plan result is None={plan is None}", file=sys.stderr)
    if plan:
